...
def profile_page_validation(name,age,username,password,username_value):
    ...
    #runs else if try runs properly without error
        ...
        for record in records:
            if str(username) != str(username_value):
...
def update_data(WIN_edit,value_name,value_age,value_username,value_password,id_record,records_user,records_profile):
    #if then Statement and calls profile_page_validation which either returns True or False
    if profile_page_validation(value_name,value_age,value_username,value_password,value_username):
        ...
        #Runs if suspected code runs well
            ...
            WIN_edit.destroy()
            from profile import profile_view as view_profile
            view_profile()
...
def profile_edit():
    ...
    update_button = Button(WIN_edit, font=tfont_tup, justify="center", width=10, borderwidth=0, text="Update", bg="#DD3939", command=lambda : update_data(WIN_edit,name_entry.get(),age_entry.get(),username_entry.get(),password_entry.get(),record_id,records_user,records_profile))
